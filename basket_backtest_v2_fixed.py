"""
ENHANCED BASKET PORTFOLIO BACKTEST V2.0 - FINAL PATCH (PRINTS SUMMARY ALWAYS)
Compatible with Python 3.7+

- Multi-format date parsing (timestamp or DD-MM-YYYY)
- Correct per-stock stop-loss booking
- Per-stock exit price, reason, robust for all cases
- Summary always printed after completion
- No modern Python (| None) type hints
"""
...
def run_basket_backtest_v2(
    ...
    print("=" * 100)
    print("BASKET PORTFOLIO BACKTEST V2.0 - FINAL PATCH")
    print("=" * 100)
    print(f"\nConfiguration:")
    ...
    # 4) Backtest logic
    print("\n[4/8] Running backtest with correct per-stock stop booking...")
    all_trades = []
    daily_tracking = []
    skipped_baskets = 0
        ...
        for day in range(1, max_hold_days + 1):
            ...
            for s in active_stocks:
                ...
                if ret <= -stop_loss_pct:
                    realized_exit_value[s] = ent * (1 - stop_loss_pct / 100)
            ...
            daily_tracking.append({
                ...
                'basket_current_sum': float(active_sum),
                'return_pct': (float(active_sum) - basket_entry_sum) / basket_entry_sum * 100.0,
...
def main():
    parser = argparse.ArgumentParser(
        description='Basket Backtest V2.0 - Final Patch with per-stock exit prices and reasons',
