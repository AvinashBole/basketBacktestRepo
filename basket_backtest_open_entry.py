"""
ENHANCED BASKET PORTFOLIO BACKTEST V2.1 - OPEN PRICE ENTRY

Modified to use OPEN price for entry instead of CLOSE price
All other logic remains identical to original
"""
...
def run_basket_backtest_v2(
    ...
    print("=" * 100)
    print("BASKET PORTFOLIO BACKTEST V2.1 - AVERAGE PRICE ENTRY")
    print("=" * 100)
    print(f"\nConfiguration:")
    ...
    req_sig = ['date', 'symbol']
    req_px = ['symbol', 'date', 'open', 'close']  # CHANGED: Added 'open' requirement
    ...
    # 4) Backtest logic
    print("\n[4/8] Running backtest with AVERAGE of open/close entry...")
    all_trades = []
    daily_tracking = []
    skipped_baskets = 0
    ...
    for i, row in groups.iterrows():
        ...
        # CHANGED: Use average of 'open' and 'close' for entry
        ...
        # CHANGED: Use average of open and close for entry prices
        entry_prices_df['avg_price'] = (entry_prices_df['open'] + entry_prices_df['close']) / 2
        entry_map = dict(zip(entry_prices_df['symbol'], entry_prices_df['avg_price']))
        ...
        for day in range(1, max_hold_days + 1):
            ...
            # Track stocks that hit individual stop loss
            to_remove = []
            ...
            # Calculate basket value including stopped stocks at their realized values
            ...
            # Check target and stop loss using corrected basket value
    ...
    print("\n[8/8] Saving outputs...")
    trades_df.to_csv('basket_trade_analysis_detailed.csv', index=False)
    print("  ✓ basket_trade_analysis_detailed.csv")

    daily_df.to_csv('basket_portfolio_daily_tracking.csv', index=False)
    print("  ✓ basket_portfolio_daily_tracking.csv")

    summary_df.to_csv('basket_strategy_summary.csv', index=False)
    print("  ✓ basket_strategy_summary.csv")

    prob_df.to_csv('basket_probability_analysis.csv', index=False)
    print("  ✓ basket_probability_analysis.csv")

    monthly_df.to_csv('basket_monthly_performance.csv', index=False)
    print("  ✓ basket_monthly_performance.csv")
...
def main():
    parser = argparse.ArgumentParser(
        description='Basket Backtest V2.1 - Open Price Entry',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python basket_backtest_open_entry.py signals.csv prices.csv
  python basket_backtest_open_entry.py signals.csv prices.csv --max-hold-days 0
        """
    )
