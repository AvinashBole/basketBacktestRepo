...
# Ensure we can import indicators from the US_buyonsupport directory
sys.path.append('US_buyonsupport')
try:
...
def run_elite_scaling_backtest(stocks, initial_capital=100000, elite_thresh=88, swap_hurdle=20):
    all_dates = sorted(pd.concat([df.index.to_series() for df in stocks.values()]).unique())
    positions = {} # {symbol: {entry_date, entry_price, peak, initial_val}}
    ...
    for current_date in all_dates:
        ...
        if len(positions) > 0 or removals or additions:
            # Format Breakdown: Sym[In:10.0,Cur:12.0,PnL:20.0%]
            breakdown = []
...
if __name__ == "__main__":
    print("Initializing Elite Scaling Backtest...")
    stocks = load_universe()
    print("Running Backtest with Detailed Reporting...")
    df_ledger = run_elite_scaling_backtest(stocks)
    
    os.makedirs('results_final', exist_ok=True)
    output_path = 'results_final/elite_scaling_detailed_ledger.csv'
