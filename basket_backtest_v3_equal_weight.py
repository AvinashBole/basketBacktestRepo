...
def run_basket_backtest_v2(
    ...
    for i, row in groups.iterrows():
        ...
        # EQUAL-WEIGHTING: Calculate quantities so each stock has equal notional value
        num_stocks = len(ordered_stocks)
        if num_stocks > 0:
            # Each stock gets equal weight: total_notional / num_stocks
            equal_weight_per_stock = sum(entry_map.values()) / num_stocks

            # Calculate quantities: quantity = equal_weight / price
            quantity_map = {s: equal_weight_per_stock / entry_map[s] for s in ordered_stocks}

            # Basket entry sum with equal weighting
            basket_entry_sum = sum(quantity_map[s] * entry_map[s] for s in ordered_stocks)
        else:
            quantity_map = {}
            basket_entry_sum = 0

        stock_entry_prices_str = "|".join([f"{s}:{entry_map[s]:.2f}(qty:{quantity_map[s]:.4f})" for s in ordered_stocks])
